 ...
 */
class CompressionUtilUnitTest {

    private val compressionUtil = CompressionUtil()

    @Test
    @Throws(Exception::class)
    fun testGzipBase64Empty() {
        val nullBytesResult = compressionUtil.gzipBase64UrlSafe(null as ByteArray?)
        Assert.assertEquals(null, nullBytesResult)

        val nullStringResult = compressionUtil.gzipBase64UrlSafe(null as String?)
        Assert.assertEquals(null, nullStringResult)

        val emptyResult = compressionUtil.gzipBase64UrlSafe("")
    ...
    fun testGzipBase64() {
        ...
        val compressed = compressionUtil.gzipBase64UrlSafe(json)

        val expected = "H4sIAAAAAAAAALXVzY7TMBAH8Ps-RZXzbmKPPZ5Jr7wBV4TQ-ItWtNvQpBfQvjuGVkJa7aoGxYfkENvzT_yLxp8eNpuf" +
