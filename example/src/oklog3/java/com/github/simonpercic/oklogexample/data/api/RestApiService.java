...
public class RestApiService implements ApiService {
    ...
    @Override public Single<List<ShowResponse>> getShows() {
        return restApi.getShows();
    }

    @Override public Single<WatchedShowResponse> watched(WatchedRequest request) {
        return restApi.watched(request);
    }

    @Override public Single<ShowResponse> createShow(CreateShowRequest request) {
        return restApi.createShow(request);
    }

    @Override public Single<ShowResponse> deleteShow(long showId) {
        return restApi.deleteShow(showId);
    }

    @Override public Completable getShowsHeader() {
        return restApi.getShowsHeader();
    }
}
